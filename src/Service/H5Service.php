...
class H5Service extends BaseService
{
     ...
     * @return array
     * 订单类型:JSAPI--公众号支付、APP--app支付、FWC--支付宝服务窗、LETPAY-小程序
     */
...
//        $postData['order_type'] = $paymentType; //支付方式
        $postData['trade_type'] = $paymentType; //支付方式
        ...
        $postData['limit_type'] = '';//异步回调地址
        $postData['openid'] = '';//异步回调地址
        $postData['product_id'] = '';//异步回调地址
        $postData['sub_appid'] = '';//异步回调地址
        $postData['sub_openid'] = '';//异步回调地址
...
//        $postData['reserved_fy_term_sn'] = '';
        ...
        return self::request('wxPreCreate', $postData);
